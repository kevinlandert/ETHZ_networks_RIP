...
next_hop_t safe_dr_get_next_hop(uint32_t ip) {
    ...
    for (unsigned int i = 0; i < tablelength; i++) {
        if(((currEntry->subnet & currEntry->mask) == ip) && currEntry->mask > maxMlen && currEntry->cost != 16 ) {
    ...
    for (unsigned int j = 0; j < intfcount; j++ ) {
        ...
        if(currInt.enabled) {
            ...
            while(current != NULL) {
                ...
                entry->metric = current->cost; //+ currInt.cost;
       ...
       while(node != NULL) {
        ...
        for(unsigned int i = 0; i < intcount; i++) {
            ...
            if (currInt.enabled) {
                ...
                node = head;
                while(node != NULL) {
                    if (((currInt.ip & currInt.subnet_mask) == (node->subnet & node->mask)) && currInt.cost < node->cost) {
                        node->next_hop_ip = 0;
